...
* **Token Recognition:** Processes standard C identifiers, complex literals (hex, float, string), and modern keywords (e.g., `_Generic`, `nullptr`).
...
* **Constant Folding:** Computes math expressions at compile-time (e.g., $3 + 4$ becomes $7$).
...
* **Typo Correction:** Uses a 110-entry table to fix keywords (e.g., `man` $\to$ `main`) and standard library functions.
* **Format/Syntax Fixes:** Adds missing `%` in format strings and repairs wrong terminators (e.g., trailing commas).
...
* **Machine Readable:** Emits stable codes (e.g., `LEAK_POSSIBLE`) that allow the server to "humanize" messages for the UI.
